...
private:
    ...
    std::vector<std::string> getDrmModeTypes(uint32_t type);
    std::vector<std::string> getDrmModeFlags(uint32_t flags);
    std::string join(std::vector<std::string> items, std::string spacer);
